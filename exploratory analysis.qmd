...
df_us_carriers <- df_all %>% 
  filter(
    CARRIER %in% c(
      "F9", "AA", "B6", "DL", "UA", "NK", "AS"
    ),
    CLASS == "F"
  ) %>% 
  select(
    DEPARTURES_SCHEDULED, DEPARTURES_PERFORMED, SEATS, PASSENGERS,
    DISTANCE, UNIQUE_CARRIER, REGION, ORIGIN, ORIGIN_STATE_ABR,
    DEST, DEST_STATE_ABR, AIRCRAFT_TYPE, YEAR, MONTH, CLASS
  ) %>% 
...
us_airports <- c("SFO", "LAX", "JFK", "SEA", "DTW", "EWR", "SPN", "BOS", "ORD",
                 "SJC", "IAD", "HNL", "IAH", "GUM", "DFW", "LAS", "ATL")

df_china_passenger_route_departure_performed_by_year <- df_china_passenger %>% 
  group_by(YEAR, ORIGIN, DEST, AIRCRAFT) %>% 
  summarise(
  ...
  group_by(YEAR, cn_airport, us_airport, AIRCRAFT) %>% 
  summarise(
  ...
  mutate(
    load_factor_average_per_route_per_year = passengers / seats * 100,
    capacity_per_departure = seats / departures
  ) %>% 
  arrange(desc(load_factor_average_per_route_per_year))
...
plot_ly(
  ...
  x = ~departures,
  y = ~passengers,
  color = ~load_factor_average_per_route_per_year,
  size = ~seats,
  sizes = c(10, 60),
  text = ~paste(
    "Route:", cn_airport, "-", us_airport,
    "<br>Year:", YEAR,
    "<br>Load Factor:", round(load_factor_average_per_route_per_year, 1), "%"
  ),
  type = "scatter",
  mode = "markers"
)
    ...
    df_cn_us_routes_summary_shiny_table %>%
      ...
      plot_ly(
        ...
        text = ~paste(
          ...
          "<br><b>Seats:</b>", seats,
          "<br><b>Load factor:</b>",
          round(load_factor_average_per_route_per_year, 1), "%"
        ),
        hoverinfo = "text",
        type = "scatter",
        mode = "markers"
